...
void* data_logging(void* arg) {
    ...
    int temp;
    while (pthread_mutex_lock(&mutex), temp = !temperature_data_received, pthread_mutex_unlock(&mutex), temp) {
...
void init_memory_manager(size_t total_mem) {
    ...
    for (int i = 0; i < MAX_BLOCKS; i++) {
        ...
        memory_blocks[i].status = FREE; // Initialize status to FREE explicitly
    }
}
...
void* allocate_memory(size_t size) {
    ...
    for (int i = 0; i < MAX_BLOCKS; i++) {
        if (memory_blocks[i].status == FREE) {
            // Allocate memory block
            memory_blocks[i].address = address;
            memory_blocks[i].size = size;
            memory_blocks[i].status = ALLOCATED;
            used_memory += size; // Increment used_memory by the size of the allocation
            break;
        }
    }
...
void free_memory(void* ptr) {
    ...
    for (int i = 0; i < MAX_BLOCKS; i++) {
        if (memory_blocks[i].address == ptr) {
            ...
            used_memory -= memory_blocks[i].size; // Decrement used_memory by the size of the freed block
            return;
        }
    }
